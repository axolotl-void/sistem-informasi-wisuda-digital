  ...
  return (
    <div className="flex items-start gap-3 rounded-xl bg-white/[0.03] px-3 py-2.5">
      <div className={`mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-lg ${
        scan.type === "masuk" ? "bg-emerald-500/10" : "bg-orange-500/10"
      }`}>
        {scan.type === "masuk"
          ? <CheckCircle2 className="size-3.5 text-emerald-400" />
          : <XCircle className="size-3.5 text-orange-400" />
        }
      </div>
      ...
      {isDrawerOpen && inv && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 z-50 flex h-full w-full max-w-sm flex-col border-l border-white/[0.08] bg-[#080f1e] shadow-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4">
              <div>
                <h3 className="text-sm font-semibold text-white/90">Detail QR</h3>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="flex flex-col items-center gap-4 rounded-2xl border border-white/[0.07] bg-white/[0.03] p-6">
                ...
                <div className="text-center space-y-1">
                  <p className="text-[0.65rem] font-semibold uppercase tracking-wider text-white/25">Token Validasi</p>
                  <p className="font-mono text-[0.68rem] text-white/50 break-all">{inv.qrToken}</p>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-4 space-y-3">
                <p className="text-[0.65rem] font-semibold uppercase tracking-wider text-white/25">Mahasiswa</p>
                <div>
                  <p className="text-sm font-semibold text-white/80">{inv.mahasiswaNama}</p>
                  <p className="text-xs text-white/35 mt-0.5">{inv.nim} · {inv.fakultas}</p>
                </div>
                <div className="grid grid-cols-2 gap-3 pt-1">
                  {[
                    ...
                    <div key={label}>
                      <p className="text-[0.65rem] text-white/25 font-medium">{label}</p>
                      <p className="text-[0.75rem] text-white/60 font-medium mt-0.5">{value}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={label} className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3 text-center">
                    <Icon className="mx-auto size-4 text-white/20 mb-1.5" />
                    <p className="text-sm font-bold text-white/70">{value}</p>
                    <p className="text-[0.6rem] text-white/25 mt-0.5">{label}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="border-t border-white/[0.06] p-4">
              <button
                type="button"
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.04] py-2.5 text-xs font-medium text-white/50 transition-all hover:border-white/[0.12] hover:bg-white/[0.07] hover:text-white/70 cursor-pointer"
              >
