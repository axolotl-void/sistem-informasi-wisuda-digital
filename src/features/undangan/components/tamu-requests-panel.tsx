...
function RequestRow({
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 rounded-2xl border border-amber-500/15 bg-amber-500/[0.04] p-3.5"
    >
      {/* Avatar */}
      <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500/20 to-blue-500/20 border border-indigo-500/15 overflow-hidden">
        {req.foto ? (
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-white/80 truncate">{req.nama}</p>
        <p className="text-[0.65rem] text-white/30 truncate">{req.nim} · {req.sesiWisuda ?? "Sesi belum ditentukan"}</p>
      </div>

      {/* Jumlah tamu badge */}
      <div className="flex items-center gap-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20 px-2.5 py-1.5 shrink-0">
        <Users className="size-3 text-amber-400/70" />
        <span className="text-xs font-bold text-amber-400">{req.requestedTamu}</span>
      </div>
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-1.5 h-8 px-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-[0.72rem] font-bold text-emerald-400 hover:bg-emerald-500/20 hover:border-emerald-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
        >
          {approving ? (
        ...
        <button
          ...
          className="flex size-8 items-center justify-center rounded-xl border border-red-500/20 bg-red-500/[0.06] text-red-400/60 hover:bg-red-500/15 hover:text-red-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
        >
          {rejecting ? (
            <Loader2 className="size-3.5 animate-spin" />
          ) : (
...
export function TamuRequestsPanel({ onRefreshUndangan }: { onRefreshUndangan?: () => void }) {
  ...
  // Tidak tampilkan jika tidak ada request
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold text-white/80">Request Tamu Pending</span>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex size-6 items-center justify-center rounded-lg text-white/25 hover:text-white/50 transition-colors"
          >
      ...
      <AnimatePresence>
        {!isCollapsed && (
          ...
          >
            <div className="px-4 pb-4 space-y-2.5 border-t border-amber-500/10 pt-3">
              {isLoading ? (
